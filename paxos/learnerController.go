	...
	return func(w http.ResponseWriter, r *http.Request) {
		if valueAccepted == false {
			...
			if acceptInfo.N > n.AcceptedN {
				...
				//If quorum is reached, the value is accepted
				if quorum >= len(n.Node.Acceptors)/2+1 {
					...
					//Notifying each proposer that the consensus is reached
					for _, address := range n.Node.Proposers {
